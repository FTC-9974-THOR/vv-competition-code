...
public class QuantumInspiredGyroAutoBlue extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        // Turn right
        robot.setMotorTargets(robot.frontLeft.getCurrentPosition()+788, robot.frontRight.getCurrentPosition()-788);
        ...
        robot.setMotorTargets(robot.frontLeft.getCurrentPosition()-ticksForDistance(9.5), robot.frontRight.getCurrentPosition()-ticksForDistance(9.5));

        robot.update(0.2, 0.2);

        while (!isStopRequested() && robot.areMotorsBusy(true)) {
            telemetry.addData("Integrated heading", gyro.getIntegratedZValue());
            telemetry.addData("Left encoder", robot.frontLeft.getCurrentPosition());
            telemetry.addData("Right encoder", robot.frontRight.getCurrentPosition());
            //telemetry.addData("Color sensor", beaconColorSensor.argb());
            telemetry.update();
            idle();
        }
        ...
        if (beaconColorSensor.red()*8 < redThreshold && beaconColorSensor.blue()*8 > blueThreshold) {
            ...
            robot.setMotorTargets(robot.frontLeft.getCurrentPosition() - ticksForDistance(4.5), robot.frontRight.getCurrentPosition() - ticksForDistance(4.5));
        ...
        // Drive up to button
        robot.setMotorTargets(robot.frontLeft.getCurrentPosition()-ticksForDistance(8.5), robot.frontRight.getCurrentPosition()-ticksForDistance(8.5));
        ...
        if (beaconColorSensor.red()*8 < redThreshold && beaconColorSensor.blue()*8 > blueThreshold) {
            ...
            robot.setMotorTargets(robot.frontLeft.getCurrentPosition() - ticksForDistance(6), robot.frontRight.getCurrentPosition() - ticksForDistance(6));
